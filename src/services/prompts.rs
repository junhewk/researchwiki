    ...
    vec![
        SchemaResponse {
            ...
            fields: vec![
                ...
                schema_string("why_it_matters", "Why healthcare AI ethics researchers should read this (3-4 sentences)", false, Some(serde_json::Value::String(String::new()))),
                schema_number("scholarly_rigor", "integer", "0-5: 5=theoretical depth, 0=problematic reasoning", false, Some(0.0), Some(0.0), Some(5.0)),
                schema_number("novelty", "integer", "0-5: 5=paradigm-shifting, 0=no novelty", false, Some(0.0), Some(0.0), Some(5.0)),
                schema_number("relevance_score", "integer", "0-5: 5=directly addresses healthcare AI ethics, 0=not relevant", false, Some(0.0), Some(0.0), Some(5.0)),
                schema_number("practical_impact", "integer", "0-5: 5=actionable recommendations, 0=unclear implications", false, Some(0.0), Some(0.0), Some(5.0)),
                schema_number("interdisciplinary", "integer", "0-4: 4=covers ethics+CS+medicine+policy, 0=ethics only", false, Some(0.0), Some(0.0), Some(4.0)),
                schema_number("critical_concerns", "integer", "-5 to 0: 0=no concerns, -3=major methodological flaws", false, Some(0.0), Some(-5.0), Some(0.0)),
                SchemaFieldResponse {
                    name: "total_score".to_string(),
                    r#type: "string".to_string(),
                    description: "Sum of all scores (max 22), calculated if not provided".to_string(),
                    required: false,
                ...
                schema_literal("priority", "Tier1 if 18-22, Tier2 if 9-17, Tier3 if 0-8", false, None, &["Tier1", "Tier2", "Tier3"]),
            ],
        },
        SchemaResponse {
